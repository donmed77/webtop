    ...
    if (submitted) {
        return (
            <div className="text-center animate-in fade-in duration-500">
                ...
                <p className="text-white/25 text-gray-300 text-xs mt-4">Redirecting in {redirectCountdown}s...</p>
            </div>
        );
    }
        ...
        <div className="w-full">
            ...
            {rating > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <p className="text-xs text-white/30 text-gray-400 uppercase tracking-wider mb-2.5 text-center">
                        ...
                        {(rating >= 3 ? POSITIVE_TAGS : NEGATIVE_TAGS).map((tag) => (
                            <button
                                ...
                                className={`inline-flex items-center px-3 py-1.5 text-xs font-medium transition-all duration-150 cursor-pointer border ${selectedTags.includes(tag.id)
                                    ? "border-[var(--color-primary-purple)] bg-[rgba(132,55,254,0.15)] text-[var(--color-primary-purple-light)]"
                    ...
                    <textarea
                        ...
                        className="w-full bg-white/[0.03] border border-white/10 rounded-none px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-white/20 resize-none mb-1"
                        rows={2}
                    />
                    <div className="flex justify-end mb-5">
                        <span className={`text-[10px] ${comment.length >= 180 ? "text-amber-400" : "text-white/15 text-gray-300"}`}>
                            {comment.length}/200
                        </span>
                    </div>
                </div>
            ...
            <div className="space-y-3">
                <button
                    ...
                    className={`w-full py-3 rounded-none text-sm font-medium transition-all duration-200 cursor-pointer border ${rating > 0
                        ? "bg-[rgba(132,55,254,0.15)] text-[var(--color-primary-purple-light)] border-[var(--color-primary-purple)] hover:bg-[rgba(132,55,254,0.25)]"
                ...
                <button
                    onClick={handleSkip}
                    className="w-full text-center text-sm text-white/25 text-gray-400 hover:text-white/50 transition-colors cursor-pointer"
                >
                    Skip
                </button>
            </div>
...
export default function SurveyPage() {
    return (
        <BrandedPageLayout>
            <Suspense fallback={
                <div className="text-white/30 text-gray-400 text-sm text-center">Loading...</div>
            }>
                <SurveyContent />
            </Suspense>
        </BrandedPageLayout>
